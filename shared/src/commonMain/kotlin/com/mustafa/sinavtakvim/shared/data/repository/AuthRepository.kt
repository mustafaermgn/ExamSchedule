...
class AuthRepository(private val firebaseAuth: FirebaseAuth?) {
    ...
    private suspend fun syncRoleFromFirestore(email: String) {
        try {
            val firebaseUid = firebaseAuth?.currentUser?.uid.orEmpty()
            val users = firestore.collection("Users").get().documents.map { it.data<User>() }
            val user = users.firstOrNull { it.email.equals(email, ignoreCase = true) }
            if (user != null) {
                selectedRole = user.role
                selectedUserId = firebaseUid.ifBlank { user.uid }
                if (firebaseUid.isNotBlank() && user.uid != firebaseUid) {
                    firestore.collection("Users").document(firebaseUid).set(user.copy(uid = firebaseUid))
                }
            }
        } catch (_: Exception) {
        }
